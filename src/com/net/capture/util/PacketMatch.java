...
public class PacketMatch {
	...
	private static PacketMatch mpm; // 单例化
	...
	public void handlePacket(PcapPacket packet) {
		PacketMatch.totalOfIp += packet.getTotalSize() / (1024.0 * 1024.0);// 单位：MB
	...
	private void handleIp4(PcapPacket packet) {
		...
		if (PacketMatch.int2Ip(ip4.destinationToInt()).equals("255.255.255.255")) {
			// System.out.println("收到了一个广播包");
			TestFrame.lItems.add(numberOfPacket, "广播数据包");
	...
	private void handleTcp(PcapPacket packet) {
		...
		TestFrame.lItems.add(numberOfPacket, "TCP Packet");
	...
	private void handleUdp(PcapPacket packet) {
		...
		TestFrame.lItems.add(numberOfPacket, "UDP Packet");
	...
	private void handleArp(PcapPacket packet) {
		...
		TestFrame.lItems.add(numberOfPacket, "ARP Packet");
	...
	private void handleIcmp(PcapPacket packet) {
		...
		TestFrame.lItems.add(numberOfPacket, "ICMP Packet");
